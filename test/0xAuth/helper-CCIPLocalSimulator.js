...
const RouterAbi = require('@chainlink/local/abi/Router.json');
const LinkTokenAbi = require('@chainlink/local/abi/LinkToken.json');
const EVM2EVMOnRampAbi = require('@chainlink/local/abi/EVM2EVMOnRamp.json');
const EVM2EVMOffRampAbi = require('@chainlink/local/abi/EVM2EVMOffRamp.json');
...
async function requestLinkFromTheFaucet(linkAddress, to, amount) {
  ...
  const linkFaucetImpersonated = await ethers.getImpersonatedSigner(
    LINK_FAUCET_ADDRESS,
  );
  ...
  const tx = await linkToken
    .connect(linkFaucetImpersonated)
    .transfer(to, amount);
...
async function routeMessage(routerAddress, evm2EvmMessage) {
  ...
  try {
    ...
    offRamps = offRampsRaw.map(([sourceChainSelector, offRamp]) => ({
      sourceChainSelector,
      offRamp,
    }));
  } catch (error) {
    throw new Error(
      `Calling router.getOffRamps threw the following error: ${error}`,
    );
  }

  for (const offRamp of offRamps) {
    if (offRamp.sourceChainSelector == evm2EvmMessage.sourceChainSelector) {
      const evm2EvmOffRamp = new ethers.Contract(
        offRamp.offRamp,
        EVM2EVMOffRampAbi,
      );
      ...
      const offchainTokenData = new Array(
        evm2EvmMessage.tokenAmounts.length,
      ).fill('0x');

      await evm2EvmOffRamp
        .connect(self)
        .executeSingleMessage(evm2EvmMessage, offchainTokenData);
  ...
  throw new Error(
    `No offRamp contract found, message has not been routed. Check your input parameters please`,
  );
}
