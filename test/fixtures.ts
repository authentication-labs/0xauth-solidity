...
export async function deployIdentityFixture() {
	...
	await claimIssuer
		...
		.addKey(
			ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['address'], [claimIssuerWallet.address])),
			3,
			1
		);
	...
	await aliceIdentity
		.connect(aliceWallet)
		.addKey(ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['address'], [carolWallet.address])), 3, 1);
	await aliceIdentity
		.connect(aliceWallet)
