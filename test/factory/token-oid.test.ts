...
describe('IdFactory', () => {
	describe('add/remove Token factory', () => {
		it('should manipulate Token factory list', async () => {
			...
			await expect(identityFactory.connect(aliceWallet).addTokenFactory(aliceWallet.address)).to.be.revertedWith(
				'Ownable: caller is not the owner'
			);

			await expect(
				identityFactory.connect(deployerWallet).addTokenFactory(ethers.ZeroAddress)
			...
			await expect(
				identityFactory.connect(deployerWallet).addTokenFactory(aliceWallet.address)
			).to.be.revertedWith('already a factory');

			await expect(identityFactory.connect(aliceWallet).removeTokenFactory(bobWallet.address)).to.be.revertedWith(
				'Ownable: caller is not the owner'
			);

			await expect(
				identityFactory.connect(deployerWallet).removeTokenFactory(ethers.ZeroAddress)
			).to.be.revertedWith('invalid argument - zero address');

			await expect(
				identityFactory.connect(deployerWallet).removeTokenFactory(bobWallet.address)
	...
	describe('createTokenIdentity', () => {
		it('should revert for being not authorized to deploy token', async () => {
			...
			await expect(
				identityFactory
					.connect(aliceWallet)
					.createTokenIdentity(aliceWallet.address, aliceWallet.address, 'TST')
		...
		it('should revert for token address being zero address', async () => {
			...
			await expect(
				identityFactory
					...
					.createTokenIdentity(ethers.ZeroAddress, aliceWallet.address, 'TST')
			).to.be.revertedWith('invalid argument - zero address');
		});
		...
		it('should revert for owner being zero address', async () => {
			...
			await expect(
				identityFactory
					.connect(deployerWallet)
					.createTokenIdentity(aliceWallet.address, ethers.ZeroAddress, 'TST')
			).to.be.revertedWith('invalid argument - zero address');
		});
		...
		it('should revert for salt being empty', async () => {
			...
			await expect(
				identityFactory
					.connect(deployerWallet)
					.createTokenIdentity(aliceWallet.address, aliceWallet.address, '')
