...
describe('IdFactory', () => {
	it('should revert because authority is Zero address', async () => {
		...
		await expect(IdFactory.connect(deployerWallet).deploy(ethers.ZeroAddress)).to.be.revertedWith(
	...
	it('should revert because sender is not allowed to create identities', async () => {
		...
		await expect(
			identityFactory.connect(aliceWallet).createIdentity(ethers.ZeroAddress, 'salt1')
	...
	it('should revert because wallet of identity cannot be Zero address', async () => {
		...
		await expect(
			identityFactory.connect(deployerWallet).createIdentity(ethers.ZeroAddress, 'salt1')
	...
	it('should revert because salt cannot be empty', async () => {
		...
		await expect(
			identityFactory.connect(deployerWallet).createIdentity(davidWallet.address, '')
	...
	it('should revert because salt cannot be already used', async () => {
		...
		await expect(
			identityFactory.connect(deployerWallet).createIdentity(davidWallet.address, 'saltUsed')
	...
	it('should revert because wallet is already linked to an identity', async () => {
		...
		await expect(
			identityFactory.connect(deployerWallet).createIdentity(aliceWallet.address, 'newSalt')
	...
	describe('link/unlink wallet', () => {
		describe('linkWallet', () => {
			...
			it('should revert for sender wallet being not linked', async () => {
				...
				await expect(identityFactory.connect(davidWallet).linkWallet(davidWallet.address)).to.be.revertedWith(
			...
			it('should revert for new wallet being already linked', async () => {
				...
				await expect(identityFactory.connect(bobWallet).linkWallet(aliceWallet.address)).to.be.revertedWith(
			...
			it('should revert for new wallet being already to a token identity', async () => {
				...
				await expect(identityFactory.connect(bobWallet).linkWallet(tokenAddress)).to.be.revertedWith(
					'invalid argument - token address'
				);
			});

			it('should link the new wallet to the existing identity', async () => {
				const { identityFactory, aliceIdentity, aliceWallet, davidWallet } = await loadFixture(
		...
		describe('unlinkWallet', () => {
			...
			it('should revert for sender wallet attemoting to unlink itself', async () => {
				...
				await expect(identityFactory.connect(aliceWallet).unlinkWallet(aliceWallet.address)).to.be.revertedWith(
			...
			it('should revert for sender wallet being not linked', async () => {
				...
				await expect(identityFactory.connect(davidWallet).unlinkWallet(aliceWallet.address)).to.be.revertedWith(
	...
	describe('createIdentityWithManagementKeys()', () => {
		...
		describe('when the wallet is included in the management keys listed', () => {
			it('should revert', async () => {
				const { identityFactory, deployerWallet, aliceWallet, davidWallet } = await loadFixture(
					deployIdentityFixture
				);
		...
		describe('when other management keys are specified', () => {
			it('should deploy the identity proxy, set keys and wallet as management, and link wallet to identity', async () => {
				...
				await expect(
					identity.keyHasPurpose(ethers.AbiCoder.defaultAbiCoder().encode(['address'], [davidWallet.address]), 1)
				).to.eventually.be.false;
				await expect(
					identity.keyHasPurpose(ethers.AbiCoder.defaultAbiCoder().encode(['address'], [aliceWallet.address]), 1)
				).to.eventually.be.false;
			});
		});
	});
