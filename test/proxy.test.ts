...
describe('Proxy', () => {
	it('should revert because implementation is Zero address', async () => {
		...
		await expect(
			IdentityProxy.connect(deployerWallet).deploy(ethers.ZeroAddress, identityOwnerWallet.address)
	...
	it('should prevent updating to a Zero address implementation', async () => {
		...
		await expect(
			implementationAuthority.connect(deployerWallet).updateImplementation(ethers.ZeroAddress)
		).to.be.revertedWith('invalid argument - zero address');
	});
	...
	it('should prevent updating when not owner', async () => {
		...
		await expect(
			implementationAuthority.connect(aliceWallet).updateImplementation(ethers.ZeroAddress)
