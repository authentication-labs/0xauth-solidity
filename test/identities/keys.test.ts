		...
		describe('Add key methods', () => {
			describe('when calling as a non-MANAGEMENT key', () => {
				it('should revert because the signer is not a MANAGEMENT key', async () => {
					...
					await expect(aliceIdentity.connect(bobWallet).addKey(bobKeyHash, 1, 1)).to.be.revertedWith(
			...
			describe('when calling as a MANAGEMENT key', () => {
				...
				it('should revert because key already has the purpose', async () => {
					...
					await expect(aliceIdentity.connect(aliceWallet).addKey(aliceKeyHash, 1, 1)).to.be.revertedWith(
		...
		describe('Remove key methods', () => {
			describe('when calling as a non-MANAGEMENT key', () => {
				it('should revert because the signer is not a MANAGEMENT key', async () => {
					...
					await expect(aliceIdentity.connect(bobWallet).removeKey(aliceKeyHash, 1)).to.be.revertedWith(
						'Permissions: Sender does not have management key'
					);
				});
			});
			...
			describe('when calling as a MANAGEMENT key', () => {
				...
				it('should revert because key does not exists', async () => {
					...
					await expect(aliceIdentity.connect(aliceWallet).removeKey(bobKeyHash, 2)).to.be.revertedWith(
				...
				it('should revert because key does not have the purpose', async () => {
					...
					await expect(aliceIdentity.connect(aliceWallet).removeKey(aliceKeyHash, 2)).to.be.revertedWith(
