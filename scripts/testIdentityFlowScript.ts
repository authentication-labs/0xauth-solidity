...
async function _deploy(hre: HardhatRuntimeEnvironment) {
  ...
  const testWallet = claimIssuerWallet

  console.log('-> Step : ID factory OP_SEPOLIA: Create identity With Management Keys');   
  const tx_createIdentity = await instance_factory.createIdentityWithManagementKeys(testWallet, formattedAddress1, 'saltlake', [ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['address'], [deployerWallet])) ]);
  await tx_createIdentity.wait();

  // const tx_createIdentity = await instance_factory.createIdentity(bobWallet, formattedAddress1, 'saltsea');
  // await tx_createIdentity.wait();
