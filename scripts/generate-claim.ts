...
async function main() {
    ...
    const claimTopic = ethers.utils.id("1010102xxxxxxx");
    const claimData = "1"; // Accredited

    const digest = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(
            ['address', 'bytes32', 'bytes'],
            [userIdentityAddress, claimTopic, ethers.utils.toUtf8Bytes(claimData)]
        )
    );
    ...
    const issuedClaim = {
        ...
        data: ethers.utils.hexlify(ethers.utils.toUtf8Bytes(claimData)),
        signature: signature,
