...
async function _deploy(hre: HardhatRuntimeEnvironment) { 
    await deploy_fixture_OP_SEPOLIA(hre); 
}
async function CONTRACT_CONFIG() {
  ...
  // const ccipRouterAddressBase = `0xE1053aE1857476f36A3C62580FF9b016E8EE8F6f`;
  // const ccipChainSelectorBase = 13264668187771770619n;
...
async function deploy_fixture_OP_SEPOLIA(
  hre: HardhatRuntimeEnvironment,
) {
...
const tx_createIdentity = await instance_factory.createIdentityWithManagementKeys.populateTransaction(bobWallet, 'bobWallet', [ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['address'], [deployerWallet])) ], {gasLimit: 1000000});
// await tx_createIdentity.wait();
const sendTx = await deployerSigner.sendTransaction(tx_createIdentity)
console.log("sendTx",sendTx)

console.log("tx_createid",tx_createIdentity)
